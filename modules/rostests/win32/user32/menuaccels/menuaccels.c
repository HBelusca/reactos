    ...
    _In_ LPCWSTR pszString)
{
    ...
    {
        LoadStringW(g_hInst, PtrToUlong(pszString), szMenu, ARRAYSIZE(szMenu));
        pszString = szMenu;
    }
...
static void dump_printf(const CHAR *fmt, ...)
{
    ...
    va_list va;
    va_start(va, fmt);
    len = _vsnprintf(szText, ARRAYSIZE(szText) - 2, fmt, va);
    if (len < 0)
        *szText = ANSI_NULL;
    va_end(va);

    /* Ensure the line ends with CR-LF */
    len = min(len, ARRAYSIZE(szText) - 3);
    szText[len + 0] = '\r';
    szText[len + 1] = '\n';
...
MD_OnKey(_In_ CHAR Pfx, _In_ HWND hwnd, _In_ BOOL fDown, _In_ WPARAM wParam, _In_ LPARAM lParam)
{
    ...
    CHAR szVkName[100];
    GetVkName(vk, szVkName, ARRAYSIZE(szVkName));
