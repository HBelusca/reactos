                  ...
                  IN LPARAM lParam)
{
    ...
    if (List)
    {
        for (i = 0; List[i]; i++)
        {
            ...
            {
                if (!co_IntSendMessage(WndChild, WM_QUERYENDSESSION, 0, lParams))
                {
            ...
            {
                co_IntSendMessage(WndChild, WM_ENDSESSION, KillTimers, lParams);
                if (KillTimers)
                {
    ...
    if (wParam & MCS_QUERYENDSESSION)
    {
        if (!co_IntSendMessage(pWindow, WM_QUERYENDSESSION, 0, lParams))
        {
            lResult = MCSR_DONOTSHUTDOWN;
        }
    }
    ...
    {
        co_IntSendMessage(pWindow, WM_ENDSESSION, KillTimers, lParams);
        if (KillTimers)
        {
