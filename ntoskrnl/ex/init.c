    ...
    _In_ PLOADER_PARAMETER_BLOCK LoaderBlock)
{
    ...
    {
        /* Skip the first two images (NTOS kernel and HAL), their
         * symbols have already been loaded by KdInitSystem() */
        if (i < 2)
            continue;
        ...
        if (LdrEntry->FullDllName.Buffer[0] == L'\\')
        {
            ...
            {
                /* Do cheap Unicode to ANSI conversion of the name */
                Count = 0;
                do
                {
        ...
        {
            /* Safely print the string into the buffer */
