                ...
                IN OUT PLDR_DATA_TABLE_ENTRY* KernelDTE)
{
    ...
    if (OperatingSystemVersion >= _WIN32_WINNT_WIN2K)
    {
        ...
        else
        {
            ...
            {
#ifndef UEFIBOOT
#if defined(SARCH_PC98) // FIXME
                {"PC-98", "bootvid.dll"},
#else
                {"PC-98", "pc98bvid.dll"},
#endif
#if defined(SARCH_XBOX) // FIXME
                {"Xbox", "bootvid.dll"},
#else
                {"Xbox", "xboxbvid.dll"},
#endif
            ...
            RtlStringCbCopyA(BootVidName, sizeof(BootVidName), "bootvid.dll");

            /* Attempt auto-detection by System ID */
            PCONFIGURATION_COMPONENT_DATA Entry = NULL;
            Entry = KeFindConfigurationEntry(LoaderBlock->ConfigurationRoot,
                                             SystemClass,
                                             ArcSystem,
                                             NULL);
            if (!Entry)
            ...
            if (Entry)
            {
                ...
                for (ULONG i = 0; i < RTL_NUMBER_OF(BootVidNames); ++i)
                {
                    ...
                    {
                        RtlStringCbCopyA(BootVidName, sizeof(BootVidName), BootVidNames[i].ModuleName);
                        break;
                    }
                }
            }
            ...
            if (Entry)
            {
                     ...
                     ((ResourceList->Version == 1) && (ResourceList->Revision > 1)) ))
                {
                    ...
                    {
                        /* It does, set the BootVid file name */
                        RtlStringCbCopyA(BootVidName, sizeof(BootVidName), "lfbbvid.dll");
                    }
                }
            }
