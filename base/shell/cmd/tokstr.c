...
const TCHAR Delimiters[] = STANDARD_SEPS;

// FIXME: Rename this helper!!
#define mystrchr(str, chr) \
    ...
    IN LPCTSTR OptsDelims2 OPTIONAL)
{
     ...
     */
    if (_istspace(Char) || mystrchr(WSpaceSeps, Char))
    {
        /*
         * ... and is NOT a specified option delimiter to be included
         * either as an explicit token or within a token, succeed.
         */
        if ( !((Flags & TS_DELIMS1_MASK) && mystrchr(OptsDelims1, Char)) &&
             !((Flags & TS_DELIMS2_MASK) && mystrchr(OptsDelims2, Char)) )
        {
            return TRUE;
        }
    }
    ...
    IN LPCTSTR OptsDelims2 OPTIONAL)
{
    ...
    while (*src)
    {
        ...
        while (*src)
        {
             ...
             */
            DelimFlags =  (mystrchr(OptsDelims1, *src) ?
                                (1 | (Flags & TS_DELIMS1_MASK))
                        : (mystrchr(OptsDelims2, *src) ?
