...
//
// FIXME: SP_COPY_SOURCEPATH_ABSOLUTE and SP_COPY_SOURCE_ABSOLUTE should not be done here!!
//

    /* find the SourceDisksFiles entry */
    if(!SetupDiGetActualSectionToInstallW(hinf, L"SourceDisksFiles", SectionName, MAX_PATH, NULL, NULL))
        return;
    ...
    //
    // FIXME: Look at the subdir too! (field # 2)
    //

    /* now find the diskid in the SourceDisksNames section */
    if(!SetupDiGetActualSectionToInstallW(hinf, L"SourceDisksNames", SectionName, MAX_PATH, NULL, NULL))
        return;
...
static __inline
BOOL equal_str(const WCHAR *a, const WCHAR *b)
{
...
#ifdef __REACTOS__
    struct file_queue *queue;
    LPWSTR src_root, src_descr, src_tag;
#else
...
#ifdef __REACTOS__
    if (params->cbSize != sizeof(*params))
    {
        SetLastError(ERROR_INVALID_PARAMETER);
        return FALSE;
    }

    queue = params->QueueHandle;
#endif
...
#ifdef __REACTOS__
    src_root  = params->SourceRootPath ? strdupAtoW( params->SourceRootPath ) : NULL;
    src_descr = strdupAtoW( params->SourceDescription );
    src_tag   = strdupAtoW( params->SourceTagfile );
#endif

    op->media = get_source_media( queue, src_root ? src_root : L"",
                                  src_descr, src_tag );
...
//
// FIXME: Convert the strings and call SetupQueueDefaultCopyW instead !!
//
                                    ...
                                    PCWSTR dst_file, DWORD style )
{
    ...
    // if (!hlist) /* Always TRUE */ hlist = hinf;
...
{
    WCHAR src_file[MAX_PATH], dst_file[MAX_PATH], *dest_dir;
    INFCONTEXT context;
    SP_FILE_COPY_PARAMS_W params;
#ifdef __REACTOS__
    ...
    if (!hlist) hlist = hinf;
#ifndef __REACTOS__ // BugBug Wine!
    if (!hinf) hinf = hlist;
#endif
    ...
    security_key = MyMalloc( (strlenW( section ) + strlenW( L".Security" ) + 1) * sizeof(WCHAR) );
    if (!security_key)
    {
        SetLastError(ERROR_NOT_ENOUGH_MEMORY);
        return FALSE;
    ...
    ret = SetupFindFirstLineW( hlist, security_key, NULL, &security_context );
    MyFree(security_key);
    if (ret)
    {
...
#endif
#ifdef __REACTOS__
    if (!(params.TargetDirectory = dest_dir = get_destination_dir( hlist, section ))) goto end;
#else // BugBug Wine!
    if (!(params.TargetDirectory = dest_dir = get_destination_dir( hinf, section ))) goto end;
#endif
    do
    {
        ...
        if (!SetupGetStringFieldW( &context, 2, src_file, ARRAY_SIZE( src_file ), NULL ))
            *src_file = 0;
        params.SourceFilename = *src_file ? src_file : dst_file;

#ifdef __REACTOS__
        params.CopyStyle = style;
        if (SetupGetIntField( &context, 4, &flags ))
        {
            if (flags & COPYFLG_WARN_IF_SKIP)
                params.CopyStyle |= SP_COPY_WARNIFSKIP;
            if (flags & COPYFLG_NOSKIP)
                params.CopyStyle |= SP_COPY_NOSKIP;
            if (flags & COPYFLG_NOVERSIONCHECK)
                params.CopyStyle &= ~SP_COPY_NEWER_OR_SAME;
            if (flags & COPYFLG_FORCE_FILE_IN_USE)
                params.CopyStyle |= SP_COPY_FORCE_IN_USE | SP_COPY_IN_USE_NEEDS_REBOOT;
            if (flags & COPYFLG_NO_OVERWRITE)
                params.CopyStyle |= SP_COPY_FORCE_NOOVERWRITE;
            if (flags & COPYFLG_NO_VERSION_DIALOG)
                params.CopyStyle |= SP_COPY_FORCE_NEWER;
            if (flags & COPYFLG_OVERWRITE_OLDER_ONLY)
                params.CopyStyle |= SP_COPY_NEWER_ONLY;
            if (flags & COPYFLG_REPLACEONLY)
                params.CopyStyle |= SP_COPY_REPLACEONLY;
            if (flags & COPYFLG_NODECOMP)
                params.CopyStyle |= SP_COPY_NODECOMP;
            if (flags & COPYFLG_REPLACE_BOOT_FILE)
                params.CopyStyle |= SP_COPY_REPLACE_BOOT_FILE;
            if (flags & COPYFLG_NOPRUNE)
                params.CopyStyle |= SP_COPY_NOPRUNE;
        }
#else
...
#ifdef __REACTOS__
//
// Add support for LZ compressed files. Fixes ATI Radeon driver installation by INF.
// See CORE-10000 for more details.
//
    ...
    if (style & (SP_COPY_NEWER_OR_SAME | SP_COPY_NEWER_ONLY | SP_COPY_FORCE_NEWER))
    {
        ...
        TRACE("SizeTarget %i ... SizeSource %i\n",
              VersionSizeTarget, VersionSizeSource);
    ...
    if (docopy)
    {
            ...
            (style & SP_COPY_IN_USE_NEEDS_REBOOT))
        {
            ...
            {
#ifdef __REACTOS__
                rc = MoveFileExW(TempFile, temp_file, MOVEFILE_REPLACE_EXISTING);
#else
...
//
// TODO: Also handle SP_COPY_SOURCEPATH_ABSOLUTE !!
//
        ...
        for (op = queue->copy_queue.head; op; op = op->next)
        {
            ...
            else
            {
                ...
                while (op_result == FILEOP_DOIT || op_result == FILEOP_NEWPATH)
                {
                    if (queue_copy_file(
#ifdef __REACTOS__
                                        op_result == FILEOP_NEWPATH ? newpath : paths.Source,
#else
                                        paths.Source,
#endif
